...
from schedulescaleopt.opt.hlcpa import HorizontalLatencyClusterPodOptimizer
from schedulescaleopt.opt.hlpa import HorizontalLatencyPodOptimizerSpec, HorizontalLatencyPodOptimizer
from schedulescaleopt.result.hlcpa.rds import RedisHlcpaResultConsumer
...
def read_hlcpa_spec_from_env(fn_name: str, lookback: int, deployment_pattern: str) -> HorizontalLatencyPodOptimizerSpec:
    pattern = 'schedule_scale_opt_hlcpa'
    ...
    time_measure = os.environ.get('schedule_scale_opt_hlcpa_time_measure_target')
...
def read_all_hlcpa_parameters_from_env(lookback: int, deployment_pattern: str) -> Dict[
    str, HorizontalLatencyPodOptimizerSpec]:
    specs = {}
    for key in os.environ.keys():
        pattern = 'schedule_scale_opt_hlcpa_duration_target_'
        if pattern in key:
            fn = key.replace(pattern, '')
            spec = read_hlcpa_spec_from_env(fn, lookback, deployment_pattern)
            specs[fn] = spec
    return specs
...
def main():
    ...
    try:
        ...
        optimizer = HorizontalLatencyClusterPodOptimizer(read_all_hlcpa_parameters_from_env(lookback, deployment_pattern))
        k8s_client = KubernetesClient.from_env()
        consumers = [RedisHlcpaResultConsumer(rds_client)]
