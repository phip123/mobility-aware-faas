...
from schedulescaleopt.opt.hcpa import HorizontalClusterPodOptimizer
from schedulescaleopt.opt.hpa import HorizontalPodOptimizer, HorizontalPodOptimizerSpec
from schedulescaleopt.result.hcpa.rds import RedisHcpaResultConsumer
...
def read_hpa_spec_from_env(fn_name: str, lookback: int, deployment_pattern: str) -> HorizontalPodOptimizerSpec:
    pattern = 'schedule_scale_opt_hcpa'
...
def read_all_hpa_parameters_from_env(lookback: int, deployment_pattern: str) -> Dict[str, HorizontalPodOptimizerSpec]:
    ...
    for key in os.environ.keys():
        pattern = 'schedule_scale_opt_hcpa_cpu_target_'
        if pattern in key:
...
def main():
    ...
    try:
        ...
        optimizer = HorizontalClusterPodOptimizer(read_all_hpa_parameters_from_env(lookback, deployment_pattern))
        k8s_client = KubernetesClient.from_env()
        consumers = [RedisHcpaResultConsumer(rds_client)]
